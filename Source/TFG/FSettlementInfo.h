...
struct FSettlementInfo
{
	...
	FSettlementInfo(): FSettlementInfo(FIntPoint(-1), TArray<FIntPoint>(), TArray<FProductionElement>(), 0.0, ESettlementState::None) {}

	FSettlementInfo(const FIntPoint& Pos, const TArray<FIntPoint>& Tiles, const TArray<FProductionElement>& Queue, const float Yield, const ESettlementState SettlementState)
	{
		Pos2D = Pos;
		OwnedTiles = Tiles;
